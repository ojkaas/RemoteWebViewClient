...
- **Clicks that survive a bad WiFi link (0.4.2)** — a short, still press is sent as one atomic `Tap` packet (the server dispatches touchStart+touchEnd back to back) instead of Down and Up as two packets that a link stall could spread seconds apart (Chromium then saw a long press, not a click). Presses that move or are held longer than 350 ms are promoted to Down/Move/Up as before. The panel also draws a translucent disc at the touch point itself (`touch_feedback`), so a press is acknowledged instantly even when the response frame takes a second. Also fixes finger-down/up being sent twice per press.
...
| `lossless_max_ratio`    | float     | ❌       | `0.2`                             | Lossless RGB565+deflate instead of JPEG for rects that compress to at most this fraction of their raw size (flat UI, text: 3-4x smaller than JPEG q100 and pixel-exact; photo-like rects stay JPEG). `0` disables. Decoded with the ESP32-P4 ROM inflater. Needs server 1.1.29+. |
| `touch_feedback`        | bool      | ❌       | `true`                            | Panel-drawn translucent disc at the touch point the moment a finger lands (before any frame comes back), so a press on a slow link is visibly registered. Costs a full-screen RGB565 shadow buffer in PSRAM. |
